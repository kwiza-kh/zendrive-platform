...
export default function CarDetail() {
  ...
  if (car === false) return <div className="container-zen py-24 text-center"><h2 className="section-title">Car not found</h2><Link to="/cars" className="btn-primary mt-6">Back to inventory</Link></div>;
  ...
  return (
    <div className="container-zen py-10">
      <Link to="/cars" className="inline-flex items-center gap-2 text-sm font-semibold text-ink-700 hover:text-accent mb-6"><FiArrowLeft /> Back to inventory</Link>

      <div className="grid lg:grid-cols-[1.3fr_1fr] gap-10">
        {/* Gallery */}
        <div>
          <div className="rounded-2xl overflow-hidden bg-ink-900 shadow-soft">
            <img src={resolveImage(shown)} alt={car.name} className="w-full h-[460px] object-cover transition" />
          </div>
          {gallery.length > 1 && (
            <div className="mt-3 grid grid-cols-5 sm:grid-cols-6 md:grid-cols-7 gap-2">
              {gallery.map((src, i) => (
                <button
                  ...
                  className={`rounded-lg overflow-hidden border-2 transition ${i === activeImg ? "border-accent" : "border-transparent hover:border-zen-line"}`}
                  type="button"
                >
              ...
              {specs.map(([Icon, k, v]) => (
                <div key={k} className="flex items-center justify-between p-4 bg-zen-bg rounded-lg border border-zen-line">
                  <div className="flex items-center gap-2.5 text-ink-700">
                    <Icon className="text-accent" /> <span className="text-sm font-semibold">{k}</span>
                  </div>
        ...
        <aside className="lg:sticky lg:top-24 h-fit space-y-6">
          <div className="card p-7">
            <p className="text-xs uppercase tracking-widest text-ink-500 font-semibold">{car.brand?.name}</p>
            <h1 className="font-display text-4xl text-ink-900 leading-tight mt-1">{car.name}</h1>
          ...
          <div className="card p-7">
            ...
            {sent ? (
              <div className="bg-accent/10 border border-accent/30 text-accent p-4 rounded-lg text-sm font-semibold text-center">
